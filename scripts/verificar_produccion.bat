@echo off
REM Posicionarse en la raíz del proyecto (un nivel por encima de scripts/).
cd /d "%~dp0\.."
...
python -c "from crm.models import Lead; print(f'Leads en DB: {Lead.objects.count()}')"
if errorlevel 1 (
...
echo.
echo Proximos pasos:
echo   1. Crear backup: scripts\backup_db.bat
echo   2. Iniciar servidor: iniciar-erp.bat (o python manage.py runserver)
